...
class Client_model extends CI_Model {
	...
	*/
	...
	function get_inspection_list($user_id){
	...
	function get_pending_form_list($client_id){
		$this->db->select("e.*, i.id as inspection_id, i.schedule_date, i.client_id");
        	
        
		$where = "i.client_id = ".$client_id;
		$this->db->where($where);
		//$this->db->where("e.client_status = 0");
	    $this->db->join('inspection as i', "i.id = e.inspection_id", 'left');
	    $data = $this->db->get( 'eform as e')->result();
	    return $data;
	}
